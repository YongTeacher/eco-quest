...
4. `migrations/0001_eco_quest.sql`과 `migrations/0002_student_roster.sql`을 D1에 적용합니다.
...
UTF-8 CSV의 첫 행에 `반,번호,이름,모둠` 열을 사용합니다. `상태` 열은 선택이며 `전학`, `중지`,
`비활성`, `제외`는 로그인 불가 학생으로 처리합니다. 명단 등록은 같은 반·번호의 행을 갱신하며,
CSV에 없는 기존 명단 행은 자동 삭제하지 않습니다.
